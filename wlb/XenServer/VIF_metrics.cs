  ...
  public class VIF_metrics : XenObject<VIF_metrics>
  {
    ...
    public override string SaveChanges(Session session, string opaqueRef, VIF_metrics server)
    {
      ...
      if (!Helper.AreEqual2<Dictionary<string, string>>(this._other_config, server._other_config))
        VIF_metrics.set_other_config(session, opaqueRef, this._other_config);
      return (string) null;
    }
    ...
    {
      session.proxy.vif_metrics_set_other_config(session.uuid, _vif_metrics != null ? _vif_metrics : "", (object) Maps.convert_to_proxy_string_string(_other_config)).parse();
    }
